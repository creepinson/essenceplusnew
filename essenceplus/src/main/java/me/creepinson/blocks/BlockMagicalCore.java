...
public class BlockMagicalCore extends ModBlocks {
	...
	public BlockMagicalCore(Material mat, String name, CreativeTabs tab, float hardness, float resistance, int harvest,
			String tool) {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			...
			if (te1 instanceof TileEntityPedastal_Magic && te2 instanceof TileEntityPedastal_Magic
					&& te3 instanceof TileEntityPedastal_Magic && te4 instanceof TileEntityPedastal_Magic) {
				if (((TileEntityPedastal_Magic) te1).getStack().getItem() == Items.BONE
						&& ((TileEntityPedastal_Magic) te2).getStack().getItem() == Items.BONE
						&& ((TileEntityPedastal_Magic) te3).getStack().getItem() == Items.BONE
						&& ((TileEntityPedastal_Magic) te4).getStack().getItem() == Items.BONE) {
					...
					EntityItem itemResult1 = new EntityItem(worldIn, pos.getX(), pos.getY() + 1, pos.getZ(),
							new ItemStack(me.creepinson.handlers.ItemHandler.boneLarge, 1));
					worldIn.spawnParticle(EnumParticleTypes.CLOUD, pos.getX(), pos.getY() + 1, pos.getZ(), 0.0D, 0.0D,
							0.0D);
					...
					playerIn.sendMessage(new TextComponentTranslation(
							TextFormatting.AQUA + "[EssencePlus] " + TextFormatting.RED + "Invalid Recipe!"));
				}
			} else {
				playerIn.sendMessage(new TextComponentTranslation(
						TextFormatting.AQUA + "[EssencePlus] " + TextFormatting.RED + "No Pedastals Found!"));
			}
		}
